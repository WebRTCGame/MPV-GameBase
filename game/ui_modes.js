...
var BuildTowerMode = function () {
	...
	this.is_legal = function (x, y) {
		...
		if (!can_build_here(gpos.gx, gpos.gy))
			return false;
	...
	this.can_enter_mode = function (x, y) {
	//console.log(this.tower.cost);
	return (SET.gold >= this.cost);
		//if (SET.gold >= this.cost)
			//return true;
		//else
			//return false;
	};
...
var TowerSelectMode = function () {
	...
	this.tear_down = function () {
		...
		if (this.killzone)
			this.killzone.is_dead = function () {
				return true;
			};
	};
	this.can_enter_mode = function (x, y) {
...
var PauseMode = function () {
	...
	this.set_up = function () {
		this.began_at = ((new Date).getTime() - start);
		$('').trigger("PauseMode");
	}
	this.tear_down = function () {
		var elapsed = ((new Date).getTime() - start) - this.began_at;
