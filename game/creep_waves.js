var CreepWaveController = function () {
	...
	cwc.delay = 25000;
	cwc.last = ((new Date).getTime() - start) - 20000;
	cwc.wave = 1;
	cwc.spawn_wave = function (bonus) {
		...
		if (this.wave % 15 === 0) {
			...
			mixins.push(StrongMixin);
		else if (this.wave % 3 === 0)
			mixins.push(QuickMixin);
	...
	assign_to_depth(cwc, SET.zIndex.system);//SET.system_render_level);
	return cwc;
};
...
var CreepWave = function (settings) {
	...
	assign_to_depth(cw, SET.zIndex.system);//SET.system_render_level);
